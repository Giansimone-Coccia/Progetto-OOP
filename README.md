...
dati da Postman su un file JSON locale. Questo salvataggio viene eseguito ogni trenta minuti, utilizzando la libreria "Timer" di Java. Inoltre, tra i vari valori che abbiamo deciso di salvare, abbiamo considerato il tempo, indicato come 'dt' nel file JSON riportato preceentemente e il valore della pressione 'pressure', così da semplificare poi la successiva lettura del file locale per il calcolo delle statistiche. Un esempio del file che viene salvato in locale:                                                                        
```
